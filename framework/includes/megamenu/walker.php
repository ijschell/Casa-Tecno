<?php
/**
 * Class Name: smarket_bootstrap_navwalker
 ...
 */
if( !class_exists('Smarket_navwalker')){
    class Smarket_navwalker extends Walker_Nav_Menu {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
                ...
                // Login/Logout link
                if( $this->enable_login_logout && smarket_is_wc() ){
                    ...
                    if( class_exists( 'WOOCS_STORAGE' ) ){
                        ...
                        foreach( $currencies as $key => $currency){
                            if( $key == $current_currency ){
                                $item_output .= '<span class="current-sym">'.esc_html__('Currency:','smarket').'</span>';
                            }
                        }
                ...
                if( $this->enable_login_logout ){
                    if( ! empty( $args->link_before ) )
                        $item_output .= $args->link_before;
                    if( is_user_logged_in () ){
                        $item_output .= ''.esc_html__('Logout','smarket');
                    }else{
                        $item_output .= ''.esc_html__('Login','smarket');
                    }
                    if( ! empty( $args->link_after ) )
                        $item_output .= $args->link_after;
                }
                ...
                if( $this->enable_minicart){
                    $cart_count = 0;
                    if(smarket_is_wc()){
                        $cart_count = smarket_get_cart_count();
                    }
                ...
                if( $this->enable_minicart){
                    $cart_content = '';
                    if(smarket_is_wc()){
                        $cart_content = smarket_get_cart_content();
                    }
                ...
                if( 'megamenu' == $item->object && $depth == 0){
                    ...
                    $smarket_mega_menu_bg_image = smarket_get_post_meta($megamenu_item->ID,'smarket_mega_menu_bg_image','');
                    $smarket_megamenu_style = smarket_get_post_meta($megamenu_item->ID,'smarket_megamenu_style','default');
                    $style ='';
                    if( $smarket_mega_menu_bg_image ){
                        $image = wp_get_attachment_image_src( $smarket_mega_menu_bg_image, 'full');
                        if( $image && is_array($image) && isset($image[0]) && $image[0] != ''){
                            $style = "background-image: url('".esc_url($image[0])."');";
                        }
                    }
                    $item_output.='<span class="toggle-submenu"></span>';
                    $item_output .= '<div style="width:'.$mega_menu_width.'px; '.$style.'" class="submenu megamenu '.$smarket_megamenu_style.'">'.do_shortcode( $megamenu_item->post_content ).'</div>';
                }
        ...
        public static function fallback( $args ) {
            if ( current_user_can( 'manage_options' ) ) {
                ...
                $fb_output .= '>';
                $fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . esc_html__( 'Add a menu', 'smarket' ) . '</a></li>';
                $fb_output .= '</ul>';
                if ( $container )
