...
if ( !class_exists( 'Smarket_MegaMenu' ) ) {
	class Smarket_MegaMenu
	{
		...
		function register_scripts( $hook )
		{
			...
			wp_enqueue_style( 'smarket-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/css/megamenu.min.css' );
			...
			wp_enqueue_media();
			wp_enqueue_script( 'smarket-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/js/megamenu.min.js', array( 'jquery' ), '1.0.0', true );
			...
			wp_localize_script( 'smarket-megamenu', 'smarket_ajax_backend', array(
				'ajaxurl'  => admin_url( 'admin-ajax.php' ),
				'security' => wp_create_nonce( 'smarket_ajax_backend' ),
			)
			);
		}
		...
		function includes()
		{
			include_once( get_template_directory() . '/framework/includes/megamenu/nav_menu_custom_fields.php' );
			include_once( get_template_directory() . '/framework/includes/megamenu/nav_edit_custom_walker.php' );
			include_once( get_template_directory() . '/framework/includes/megamenu/walker.php' );
		}
		...
		public function set_fonts()
		{
			$this->lits_fonts = array(
				array(
					'title' => esc_html__( 'Font Awesome', 'smarket' ),
					'fonts' => array(
            ...
            <div class="icons-popup">
                <div class="inner">
					<?php if ( count( $this->lits_fonts ) > 0 ): ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e( 'No Font', 'smarket' ); ?>
					<?php endif; ?>
                </div>
            </div>
			<?php
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_smarket_shortcode_custom_css', true );
				}
				if ( !empty( $shortcodes_custom_css ) ) {
					wp_enqueue_style(
						'smarket_megamenu_shortcodes-custom-css',
						get_template_directory_uri() . '/css/custom.css'
					);

					wp_add_inline_style( 'smarket_megamenu_shortcodes-custom-css', $shortcodes_custom_css );
				}
			}
		}
