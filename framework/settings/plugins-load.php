...
if ( !class_exists( 'Smarket_PluginLoad' ) ) {
	class Smarket_PluginLoad
	{
		...
		public function plugins()
		{
			...
			$this->plugins = array(
				array(
					'name'               => 'Smarket Toolkit', // The plugin name
					'slug'               => 'smarket-toolkit', // The plugin slug (typically the folder name)
					'source'             => esc_url( 'http://kutethemes.net/wordpress/plugins/smarket-toolkit.zip' ), // The plugin source
					...
					'image'              => get_theme_file_uri( '/images/toolkit.jpg' ),
				),
				array(
					'name'               => 'Smarket Importer', // The plugin name
					'slug'               => 'smarket-importer', // The plugin slug (typically the folder name)
					'source'             => esc_url( 'http://kutethemes.net/wordpress/plugins/smarket-importer.zip' ), // The plugin source
					'required'           => true, // If false, the plugin is only 'recommended' instead of required
					'version'            => '1.0.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
					'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
					'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
					'external_url'       => '', // If set, overrides default API URL and points to an external URL
					'image'              => get_theme_file_uri( '/images/import.jpg' ),
				),
				array(
				...
				array(
					...
					'slug'     => 'meta-box',
					'required' => true,
					'image'    => esc_url( 'http://kutethemes.net/wordpress/demos/images/images-import/meta-box.jpg' ),
				),
				array(
					'name'     => 'WooCommerce',
					'slug'     => 'woocommerce',
		...
		{
			$this->config = array(
				'id'           => 'smarket',                 // Unique ID for hashing notices for multiple instances of TGMPA.
				'default_path' => '',                      // Default absolute path to bundled plugins.
				'menu'         => 'smarket-install-plugins', // Menu slug.
...
if ( !function_exists( 'Smarket_PluginLoad' ) ) {
	function Smarket_PluginLoad()
	{
		new  Smarket_PluginLoad();
	}
}
add_action( 'tgmpa_register', 'Smarket_PluginLoad' );